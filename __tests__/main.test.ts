...
describe('API testing', () => {
  ...
  test('get suggestions will return 2 matching documents', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q: 'lon', latitude: 43.70011, longitude: -79.4163, radius: 500 , sort:'name'})
      .catch((err) => { throw err });
  ...
  test('get suggestions will return no matching documents', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q: 'xxxxxx', latitude: 43.70011, longitude: -79.4163, radius: 500 , sort:'name'})
      .catch((err) => { throw err });
  ...
  test('request without q will fail', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({  latitude: 43.70011, longitude: -79.4163, radius: 500 , sort:'name'})
      .catch((err) => { throw err });
      expect(response.body).toContain('Failed to validate')

  });

  test('request with a non number value for latitude will fail', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q:'lon', latitude: 'lat', longitude: -79.4163, radius: 500 , sort:'name'})
      .catch((err) => { throw err });
      expect(response.body).toContain('Failed to validate')
  ...
  test('request with a non number value for longitude will fail', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q:'lon', latitude: 43.70011, longitude: 'long', radius: 500 , sort:'name'})
      .catch((err) => { throw err });
      expect(response.body).toContain('Failed to validate')
  ...
  test('request with a non number value for radius will fail', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q:'lon', latitude: 43.70011, longitude: -79.4163, radius: 'rad' , sort:'name'})
      .catch((err) => { throw err });
      expect(response.body).toContain('Failed to validate')
  ...
  test('request with a value for sort other than name or distance  will fail', async () => {
    const response= await request(app).get('/api/cities/suggestions')
      .query({ q:'lon', latitude: 43.70011, longitude: -79.4163, radius: 500 , sort:'sort'})
      .catch((err) => { throw err });
      expect(response.body).toContain('Failed to validate')
